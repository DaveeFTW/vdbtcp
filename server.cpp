...
{
    auto bits = 0u;
    auto res = m_eventFlag.waitForAny(CONNECTION, &bits);

    if (res < 0)
    {
        LOG("error waiting for connection (0x%08X)\n", res);
        return {};
    }

    int socket = bits & ~CONNECTION;
    return Connection(socket);
}

void Server::onNewConnection(int socket)
{
    auto res = m_eventFlag.set(CONNECTION | socket);

    if (res < 0)
    {
        LOG("error setting event flag for new connection: 0x%08X\n", res);
    }
}
